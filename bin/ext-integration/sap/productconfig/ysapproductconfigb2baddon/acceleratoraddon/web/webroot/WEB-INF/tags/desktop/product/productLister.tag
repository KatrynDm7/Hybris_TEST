...
<%@ taglib prefix="common" tagdir="/WEB-INF/tags/addons/b2bacceleratoraddon/desktop/common" %>
	...
	<c:forEach items="${pageData.results}" var="product">
		<c:choose>
			<c:when test="${itemType eq 'CATALOG' || (itemType eq 'ORDERFORM' && empty product.variantMatrix)}">
				<productConfig:productListerItem product="${product}" isOrderForm="${isOrderForm}"/>
			</c:when>
