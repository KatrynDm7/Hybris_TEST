...
<div class="prod_add_to_cart">
	...
	<c:url value="/sbgaddtocart" var="sbgAddToCartUrl"/>

	<c:if test="${product.itemType eq 'SubscriptionProduct'}">
		<form:form id="addToCartForm" class="add_to_cart_form" action="${sbgAddToCartUrl}" method="post">
            <div class="tabbody device-only">

                <input type="hidden" name="productCodePost" value="${product.code}"/>
                <input type="hidden" size="1" id="qty" name="qty" class="qty" value="1">
            ...
            <button type="${buttonType}" disabled=true class="delayed-button disabled positive large <c:if test="${fn:contains(buttonType, 'button')}">out-of-stock</c:if>">
                <spring:theme code="text.addToCart" var="addToCartText"/>
					...
					<h3><c:if test="${frequencyTab.termOfServiceNumber gt 0}">${frequencyTab.termOfServiceNumber}&nbsp;</c:if>${frequencyTab.termOfServiceFrequency.name}</h3>
						...
						<table class="sub-tabbody-table">
							...
							<c:forEach items="${frequencyTab.products}" var="plan" varStatus="plansCounter">	
							    ...
							    <tr>
                                    <td class="sub-tabbody-table-td0">${plan.name}</td>
                                    <td class="sub-tabbody-table-td1"><a class="details" href="#detail-${bundleTabCounter.count}_${frequencyTabCounter.count}_${plansCounter.count}" tabindex="0"><spring:theme code="product.overview" text="Details"/></a>
										<div class="hidden">
															...
															<c:forEach items="${plan.entitlements}" var="entitlement">
																	...
																	<c:if test="${entitlement.quantity ne null}">
																		<c:choose>
																			...
																			<c:when test="${entitlement.quantity ge 0}">
									...
									<td class="sub-tabbody-table-td4">
										...
										<div id="${plan.code}">
											...
											<button type="${buttonTypeBundle}" disabled=true class="disabled positive large delayed-button <c:if test="${fn:contains(buttonTypeBundle, 'button')}">out-of-stock</c:if>">
												<spring:theme code="text.addToCart" var="addToCartText"/>
												<spring:theme code="basket.add.to.basket" />
											</button>
										</div>
                        ...
                        <table class="item-only-table">
                            <tr>
                                ...
                                <td class="item-only-add">
                                    ...
                                    <spring:theme code="text.addToCart" var="addToCartText"/>
                                    <button type="${buttonType}" disabled=true class="positive disabled large delayed-button <c:if test="${fn:contains(buttonType, 'button')}">out-of-stock</c:if>">
                                        <spring:theme code="text.addToCart" var="addToCartText"/>
                                        <spring:theme code="basket.add.to.basket" />
                                    </button>
                                </td>
