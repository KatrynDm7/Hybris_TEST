...
<div class="summaryDeliveryMode span-8" >
	...
	<c:if test="${cartData.pickupItemsQuantity > 0}">
		<div class="column">
            <c:choose>
                <c:when test="${cartData.deliveryItemsQuantity > 0}">
                    <strong>&nbsp;</strong>
                </c:when>
                <c:otherwise>
                    <strong><spring:theme code="checkout.summary.deliveryMode.header" htmlEscape="false"/></strong>
	...
	<c:if test="${cartData.deliveryItemsQuantity > 0}">
		<ycommerce:testId code="checkout_changeDeliveryMode_element">
			<a href="<c:url value="/checkout/multi/delivery-method/choose"/>" class="editButton"><spring:theme code="checkout.summary.edit"/></a>
		</ycommerce:testId>
	</c:if>
</div>