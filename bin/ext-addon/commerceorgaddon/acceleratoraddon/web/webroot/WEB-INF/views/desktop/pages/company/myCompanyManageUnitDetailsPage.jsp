...
<template:page pageTitle="${pageTitle}">
	...
	<div class="column companyContentPane clearfix orderList">
		...
		<div class="headline"><spring:theme code="text.company.manage.units.unitDetails" text="View Unit: {0}" arguments="${empty escapedUnitName ? escapedUnitUid : escapedUnitName}"/></div>
		...
		<table class="orderListTable">
			...
			<c:forEach items="${unit.addresses}" var="address">
				<tr>
					...
					<td headers="1">${fn:escapeXml(address.line1)}</td>
					<td headers="2">${fn:escapeXml(address.line2)}</td>
					<td headers="3">${fn:escapeXml(address.town)}</td>
					<td headers="4">${fn:escapeXml(address.postalCode)}</td>
					<td headers="5">${fn:escapeXml(address.country.name)}</td>
					<td headers="6">
			...
			<table class="orderListTable">
				...
				<c:forEach items="${unit.costCenters}" var="b2bCostCenter">
					...
					<tr>
						...
						<td headers="2">${fn:escapeXml(b2bCostCenter.name)}</td>
						<td headers="3">${fn:escapeXml(b2bCostCenter.currency.name)}</td>
						<td headers="4">
