...
public class FluentSqlBuilderTest
{
	...
	public void shouldCreateInsertStatementForThreeColumnsWithPlaceholders()
	{
		...
		// when
		final String sql = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).insert().into(TABLE_NAME).usingFields("col1",
				  "col2",
				  "col3").values(values)
				  .toSql();
	...
	public void shouldCreateDeleteStatementForOneColumn()
	{
		...
		// when
		final String sql = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).delete().from(TABLE_NAME).where().field(colName)
				  .isEqual().toSql();
	...
	public void shouldCreateDeleteStatementForMoreThanOneColumn()
	{
		...
		// when
		final String sql = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).delete().from(TABLE_NAME).where().field(colName1)
				  .isEqual().and()
	...
	public void shouldCreateSelectStatementForOneColumn()
	{
		...
		// when
		final String sql = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).select(colName1,
				  colName2).from(TABLE_NAME).where().field(colName1).isEqual()
				  .toSql();
	...
	public void shouldCreateSelectStatementForMoreThanOneColumn()
	{
		...
		final String sql = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).select(colName1,
				  colName2).from(TABLE_NAME).where().field(colName1).isEqual()
				  .and().field(colName2).isEqual().toSql();
	...
	public void shouldCreateSelectDistinctStatement()
	{
		...
		// when
		final String sql = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).selectDistinct(colName1,
				  colName2).from(TABLE_NAME).where().field(colName1)
				  .isEqual().toSql();
	...
	public void shouldCreateSelectStatementForMoreThanOneColumnUsingEqualAndNotEqualOperands()
	{
		...
		final String sql = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).select(colName1,
				  colName2).from(TABLE_NAME).where().field(colName1).isEqual()
				  .and().field(colName2).isEqual().and().field(colName3).isNotEqual().toSql();
	...
	public void shouldCreateSelectStatementWithMaxFunctionOnOneOfTheColumns()
	{
		...
		// when
		final String sql = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).select(colName1,
				  max(colName2)).from(TABLE_NAME).where().field(colName1)
				  .isEqual().groupBy(colName2).toSql();
	...
	public void shouldCreateSelectStatementWithMaxFunctionOnOneOfTheColumnsUsinWhereIN()
	{
		...
		final String sql = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).select(colName1,
				  max(colName2)).from(TABLE_NAME).where().field(colName1)
				  .in(values).groupBy(colName2).toSql();
	...
	public void shouldCreateUpdateStatement()
	{
		...
		// when
		final FluentSqlBuilder builder = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).update(TABLE_NAME).set("col1",
				  "col2", "col3").where()
				  .field("col1").isEqual();
	...
	public void shouldCreateSelectStatementWithIsNullWhereClause() throws Exception
	{
		...
		// when
		final FluentSqlBuilder builder = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).select(colName1,
				  colName2).from(TABLE_NAME).where().field
				  (colName1).isNull().and().field(colName2).isNotNull();
		final String sql = builder.toSql();
	...
	{
		// given
		final FluentSqlBuilder builder1 = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).select("foo", "bar",
				  "baz").from(TABLE_NAME).where()
				  .field("foo").isEqual();
		final FluentSqlBuilder builder2 = FluentSqlBuilder.builder(Config.DatabaseName.HSQLDB).select("baz", "boom",
				  "moo").from(TABLE_NAME).where()
				  .field("moo").isEqual();
	...
	public void shouldCreateSelectStatementUsingINForOracleInCombinedWhereClause() throws Exception
	{
		...
		assertThat(sql).isEqualTo("SELECT foo FROM " + TABLE_NAME + " WHERE bar IN (" + paramsString1 + ") AND (baz," +
				  "'hy') IN (" + paramsString2 + ") OR moo IN (" + paramsString3 + ")");
	}
	...
	public void shouldCreateStatementWithOrderBy() throws Exception
	{
		...
		// when
		final String sql = FluentSqlBuilder.genericBuilder().select(colName1,
				  colName2).from(TABLE_NAME).where().field(colName2).isEqual().orderBy(colName1).toSql();
	...
	public void shouldCreateStatementWithSubQuery() throws Exception
	{
		...
		final String colName1 = "col1";
		final String colName2 = "col2";
		final FluentSqlBuilder subQuery = FluentSqlBuilder.genericBuilder().select(colName1,
				  colName2).from(TABLE_NAME).where().field(colName2).isEqual();
