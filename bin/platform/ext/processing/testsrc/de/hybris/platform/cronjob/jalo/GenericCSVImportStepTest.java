...
public class GenericCSVImportStepTest extends HybrisJUnit4TransactionalTest
{
	...
	private void testeMitEncoding(final String encoding, final List soll, final int skipLine, final boolean un) throws Exception
	{
		...
		//		ein! media erstellen
		params.clear();
		params.put(JobMedia.CODE, filename);
	...
	public class GenericCSVImportStepClon extends GenericCSVImportStep
	{
		...
		{
			return super.getCommentOutCharacters();
		}

		@Override
		protected char[] getTokenCharacters()
		{
			return super.getTokenCharacters();
		}

		@Override
		protected void performStep(final CronJob cronJob) throws AbortCronJobException, JaloInvalidParameterException
		{
		...
		@Override
		protected void checkAbort(final CronJob cronJob) throws AbortCronJobException
		{
			super.checkAbort(cronJob);
		}

		@Override
		protected CSVReader createCSVReader(final InputStream is, final String encoding) throws UnsupportedEncodingException
		{
			return super.createCSVReader(is, encoding);
		}

		@Override
		protected void finalizeStep(final CronJob cronJob) throws JaloBusinessException
		{
			super.finalizeStep(cronJob);
		}
		...
		@Override
		protected InputStream openMedia(final JobMedia media, final CronJob cronJob) throws JaloBusinessException, IOException
		{
			return super.openMedia(media, cronJob);
		}

		@Override
		protected boolean skipFirstLine()
		{
			return super.skipFirstLine();
		}
	}
}
