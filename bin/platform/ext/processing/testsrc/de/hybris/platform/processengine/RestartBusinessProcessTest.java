...
public class RestartBusinessProcessTest extends ServicelayerBaseTest
{
	...
	private void waitForAction(final BusinessProcessModel model, final String state) throws InterruptedException, TimeoutException
	{
		waitForCondition(r -> {
			...
			try
			{
				return Boolean.valueOf(model.getCurrentTasks().iterator().next().getAction().equals(state));
			}
	...
	private void waitForState(final BusinessProcessModel model, final ProcessState state) throws InterruptedException,
			TimeoutException
	{
		waitForCondition(r -> {
			modelService.refresh(model);
