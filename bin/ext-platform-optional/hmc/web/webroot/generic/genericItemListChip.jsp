...
%>
		<table class="genericItemListChip" cellspacing="0" cellpadding="0" oncontextmenu="<%= contextMenu %>" >
			<tr>
				<td>
...
%>
						<table id="table_<%= theChip.getUniqueName() %>_table" class="listtable selecttable<%= theChip.isMultipleSelectionAllowed() ? "" : " singleselect" %>" cellpadding="0" cellspacing="0">
							<tr>
								<!-- gilcHeaderCheckbox -->
...
<%
	...
	else
	{
		...
		for (final Iterator theChips = theChip.getRestrictedListEntries().iterator(); theChips.hasNext();)
		{
			...
			{
%>
								<tr id="<%= chip.getUniqueName() %>_tr" <%= prohibitOpening ? "" : "class=\"doubleclick-event " + chip.getCommandID(GenericItemListEntryChip.OPEN_EDITOR) + "\"" %> onclick="">	<%-- mr: empty onclick is necessary for selenium test recording! --%>
									<!-- gilcCheckbox -->
