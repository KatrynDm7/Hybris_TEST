...
(function(tinymce) {
	tinymce.dom.ScriptLoader = function(settings) {
		...
		function loadScript(url, callback) {
			...
			// ######  added by mr to deal with broken editor for missing plugin languages (see http://tinymce.moxiecode.com/develop/bugtracker_view.php?id=3108)
			
			if (/langs\/([a-z]{2})\.js/.test(url)) 
			{
				elm.onerror = done;
			}
