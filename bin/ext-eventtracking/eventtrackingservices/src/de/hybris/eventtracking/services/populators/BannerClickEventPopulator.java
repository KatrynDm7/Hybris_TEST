 ...
 * 
 */
public class BannerClickEventPopulator extends AbstractTrackingEventGenericPopulator
{

	public BannerClickEventPopulator(final ObjectMapper mapper)
	{
		super(mapper);
	}
	...
	{
		return BannerClickEvent.class.isAssignableFrom(clazz);
	}
			...
			throws ConversionException
	{
		...
		if (customVariablesPageScoped != null)
		{
			...
			if (cvrIndex1 != null && cvrIndex1.size() > 1 && TrackingEventJsonFields.BANNER_ID.getKey().equals(cvrIndex1.get(0)))
			{
				bannerId = cvrIndex1.get(1);
			}
		}
		((BannerClickEvent) trackingEvent).setBannerId(bannerId);
		((BannerClickEvent) trackingEvent).setEventType("BannerClickEvent");
	}
