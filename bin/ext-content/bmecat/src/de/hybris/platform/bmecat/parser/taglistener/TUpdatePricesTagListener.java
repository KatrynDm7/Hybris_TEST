...
/**
 * Parses the &lt;TUpdatePrices&gt; tag
 */
public class TUpdatePricesTagListener extends DefaultBMECatTagListener
{
	/**
	 * Dummy constructor; use this one if you need the method <code>getTagName()</code> only!
	 */
	public TUpdatePricesTagListener()
	{
		super();
	}

	public TUpdatePricesTagListener(final TagListener parent)
	{
		super(parent);
	}
	...
	protected Object processStartElement(final BMECatObjectProcessor processor) throws ParseAbortException
	{
		...
		if (cat != null)
		{
			cat.setTransactionMode(BMECatConstants.TRANSACTION.T_UPDATE_PRICES);
			cat.setPreviousVersion(getAttribute(BMECatConstants.XML.ATTRIBUTE.T_UPDATE_PRICES.PREV_VERSION) != null ? Integer
					.valueOf(getAttribute(BMECatConstants.XML.ATTRIBUTE.T_UPDATE_PRICES.PREV_VERSION)) : null);
		}
		/*
		 * notify processor
		 */
	...
	{
		return BMECatConstants.XML.TAG.T_UPDATE_PRICES;
	}
