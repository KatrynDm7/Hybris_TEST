...
<%-- We cannot use ${z:outLangStyleSheets()} since Executions.getCurrent()
 ...
 <link rel="stylesheet" type="text/css" href="../css/importcockpit.css"/>
...
<c:forEach var="cnt" begin="1" end="${maxcnt}" varStatus="s">
	<tr>
		...
		<td style="position:relative;">
			<input type="text" class="filefake2" id="faketext" value=""/>
			<input type="button" class="fd-button filefake1" value="${c:l('importcockpit.upload.select')}"/>
			<input class="file" type="file" id="file" name="file" onChange="document.getElementById('faketext').value = this.value" style="margin:0;margin-left:100px;position:relative;z-index:2;filter:alpha(opacity=0);-moz-opacity: 0; opacity: 0;"/>
			<c:if test="${unlimited && s.index == maxcnt}"><img src="${c:encodeURL('~./img/spacer.gif')}" class="z-fileupload-add z-button" onclick='addUpload(this);'"/></c:if><c:if test="${unlimited && s.index < maxcnt}"><img src="${c:encodeURL('~./img/spacer.gif')}" class="z-fileupload-delete z-button" onclick='deleteUpload(this);'"/></c:if>
		</td>
	</tr>
</c:forEach>
	</table>
	</div>
	<table border="0" width="100%" style="border-top:1px dotted #bdbdc0;margin-top:10px;">
	<tr align="left">
		<td style="position:relative;padding-top:10px;" class="fd-buttons">
			<input class="fd-button" style="float:left;" type="button" value="${c:l('mesg:org.zkoss.zul.mesg.MZul:UPLOAD_CANCEL')}" onclick="cancelUpload()"/>
			<input class="fd-button" style="float:right;" type="submit" value="${c:l('mesg:org.zkoss.zul.mesg.MZul:UPLOAD_SUBMIT')}" onclick="parent.zk.progress()"/>
		</td>
	<tr>
	</table>
	</form>
